...
from ddim.models.diffusion_dynamic_cut import ResnetBlock, AttnBlock, nonlinearity
#from ddim.models.diffusion_dynamic import ResnetBlock, AttnBlock, nonlinearity
...
class QuantResnetBlock(BaseQuantBlock):
    def __init__(
        ...
        if self.add_lora:
            self.conv1_Lora2d = res.conv1_Lora2d
        self.temb_proj = res.temb_proj
        self.norm2 = res.norm2
        self.dropout = res.dropout
        self.conv2 = res.conv2
        if self.add_lora:
            self.conv2_Lora2d = res.conv2_Lora2d
        if self.in_channels != self.out_channels:
            if self.use_conv_shortcut:
                self.conv_shortcut = res.conv_shortcut
                if self.add_lora:
                    self.conv_shortcut_Lora2d = res.conv_shortcut_Lora2d

            else:
                self.nin_shortcut = res.nin_shortcut
                if self.add_lora:
                    self.nin_shortcut_Lora2d = res.nin_shortcut_Lora2d
...
class QuantAttnBlock(BaseQuantBlock):
    def __init__(
        ...
        self.act_quantizer_v = UniformAffineQuantizer(**act_quant_params)
        if self.add_lora:
            self.q_Lora2d = attn.q_Lora2d
            self.k_Lora2d = attn.k_Lora2d
            self.v_Lora2d = attn.v_Lora2d
            self.proj_out_Lora2d = attn.proj_out_Lora2d
