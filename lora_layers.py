...
class LoRALayer(nn.Module):
    def __init__(
        ...
        if lora_dropout > 0.:
            ...
            self.dropout2d = nn.Dropout2d(p=lora_dropout)
        else:
...
class LoRA_Conv2d(nn.Conv2d, LoRALayer):
    # LoRA implemented in a convolutional layer
    def __init__(
            self,
            in_channels: int,
            out_channels: int,
            ...
            lora_dropout: float = 0.,
            merge_weights: bool = True,
            add_gate: bool = True, # add a parameter to control the gate
            **kwargs
    ):
        ...
        if r > 0:
            ...
            self.weight.requires_grad = False
        self.reset_parameters()
        self.add_gate = add_gate # store the parameter as an attribute
        if add_gate: # if add_gate is True, create a gate layer
            self.lora_gate = nn.Conv2d(in_channels, 1, kernel_size, **kwargs)
    ...
    def forward(self, x: torch.Tensor):
        if self.r > 0 and not self.merged:
            ...
            if self.add_gate: # if add_gate is True, apply the gate layer to the input and multiply it with the scaling factor
                self.scaling = torch.sigmoid(self.lora_gate(x))
                self.scaling = torch.mean(self.scaling, dim=1).unsqueeze(-1)
                result *= self.scaling
            return result
...
class LoRA_Conv2d_Adapter(LoRALayer):
    # LoRA implemented in a convolutional layer
    def __init__(
            self,
            in_channels: int,
        ...
        if add_gate: # if add_gate is True, create a gate layer
            self.lora_gate = nn.Conv2d(in_channels, 1, kernel_size, **kwargs)
        self.format = torch.zeros((out_channels, in_channels, kernel_size, kernel_size))
    ...
    def forward(self, x: torch.Tensor):
        if self.r > 0:
            ...
            result = F.conv2d(
                x,
                (self.lora_B @ self.lora_A).view(self.format.shape) * self.scaling,
                stride = self.stride, padding = self.padding, bias=False
            )
            result = self.dropout2d(result)
            if self.add_gate: # if add_gate is True, apply the gate layer to the input and multiply it with the scaling factor
                gate = torch.sigmoid(self.lora_gate(x))
                gate = torch.mean(gate, dim=(1, 2, 3), keepdim=True)
                # print(self.scaling.shape)
                # print(result.shape)
                result *= gate
            return result
